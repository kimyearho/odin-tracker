...
<el-row :gutter="20">
    ...
    <el-col style="margin-top: 20px" :xl="{span: 12}" :lg="{span: 4}" :md="{span: 6}" :sm="{span: 16}">
        <el-card shadow="always">
            ...
            <el-form class="statForm" :inline="true" size="mini" label-position="top">
                <el-form-item>
                    <template #label>레벨</template>
                    <el-input-number v-model="userLevel" :min="1" :max="90"></el-input-number>
                </el-form-item>
                <el-form-item>
                    <template #label>명중</template>
                    <el-input-number v-model="userHitRate" :min="10" :max="300"></el-input-number>
                </el-form-item>
                <el-form-item>
                    <template #label>추가 명중 (근/마/원)</template>
                    <el-input-number v-model="userCharHitRate" :min="0" :max="100"></el-input-number>
                </el-form-item>
                <el-form-item>
                    <template #label>사냥터 선택</template>
                    <el-select class="mobSelect" :model-value="mobArea" placeholder="선택" @change="changeMobArea">
                        <el-option-group v-for="group in options" :key="group.label" :label="group.label">
                            <el-option v-for="item in group.options" :key="item.value" :label="item.label" :value="item.value">
                                <span style="float: left">{{ item.label }}</span>
                                <span style="float: right; color: #8492a6; font-size: 13px">{{ item.value.min }}~{{ item.value.max }}</span>
                            </el-option>
                        </el-option-group>
                    </el-select>
                </el-form-item>
            ...
            <el-row>
                <el-col>
                    <div class="bottom">
                        <el-button class="btn" type="primary" icon="el-icon-check" size="mini" @click="hitRateResult">계산하기</el-button>
                    </div>
                </el-col>
            </el-row>
        </el-card>
        ...
        <el-card shadow="always">
            ...
            <div class="contents">
                <el-form :inline="true" size="mini" label-position="top">
                    <el-form-item>
                        <template #label>동일 레벨 적중률 (평타/스킬)</template>
                        <span class="rate">{{ hitRateAvg }} / {{ hitRateSkill }} (%)</span>
                    </el-form-item>
                </el-form>
                <el-form :inline="true" size="mini" label-position="top">
                    <el-form-item class="mobAreaHitRate">
                        <template #label>사냥터별 최소/최대 레벨간 적중률 ({{ mobArea }}) (평타/스킬)</template>
                        <el-row>
                            <el-col>
                                <span class="rate">최소: {{ mobAreaMinLevelHitRate }} / {{ mobAreaMinLevelSkillHitRate }} (%)</span>
                            </el-col>
                            <el-col>
                                <span class="rate">최대: {{ mobAreaMaxLevelHitRate }} / {{ mobAreaMaxLevelSkillHitRate }} (%)</span>
                            </el-col>
                        </el-row>
                    </el-form-item>
                </el-form>
...
<el-row class="notice">
    ...
    <el-col>
        <ol>
            <li>
                <span>이 프로그램은 오딘 인벤 명중률 실험 리포트를 참고하여 만듬. 따라서, 계산식과 기준표는 리포트에 있는것과 동일함. (https://www.inven.co.kr/webzine/news/?news=259851&site=odin)</span>
            </li>
            <li>
                <span>최대 명중률 88.88%, 최소 명중률 45.45%이하는 계산되지 않음.</span>
            </li>
            <li>
                <span>동일 레벨 명중률은 레벨업시 항상 감소함. 명중은 3의 배수이므로 레벨업시 증가하는 명중외에 최소 1을 더 채워야 떨어진 명중률이 원복됨.</span>
            </li>
            <li>
            ...
            <li>
                <span>사냥터별 계산값은 입력한 내 캐릭터 레벨기준으로 계산 됨.</span>
            </li>
            <li>
                <span>미드가르드와, 알브하임은 지원안함. 알브하임 못간 쪼렙이라 지도정보 제공해주면 추가 개발 가능함.</span>
            </li>
            <li>
                <span>개발자: (오딘05/젠붕이) | 문의 사항은 인벤쪽지로</span>
            </li>
        </ol>
    </el-col>
</el-row>
...
function calculatorService() {
    ...
    function hitRateResult() {
        ...
        if (rateMark > plusMax) {
            ElMessage({ message: '[기준치 초과] 명중률 최대 계산율인 88.89%를 초과 했음.', type: 'error' })
            return;
        }

        if (rateMark < minusMax) {
            ElMessage({ message: '[기준치 초과] 명중률 최하 계산율인 45.45%보다 낮음.', type: 'error' })
            return;
        }
    ...
    function sameHitRateResult(mark) {
        $axios.get('/hitRateList.json')
            .then(({ data }) => {
                const sameRateArray = data['data'].filter(item => item.calc_mark === mark)
    ...
    function sameMobAreaHitRateResult(mobMark, type) {
        $axios.get('/hitRateList.json')
            .then(({ data }) => {
                const sameRateArray = data['data'].filter(item => item.calc_mark === mobMark)
                if (sameRateArray.length > 0) {
                    if (type === 'min') {
                        //* 사냥터별 최소 레벨 평타 적중률
                        mobAreaMinLevelHitRate.value = sameRateArray[0]['same_rage']
                        mobAreaMinLevelSkillHitRate.value = skillHitRatePercent(mobAreaMinLevelHitRate.value)
                    } else if (type === 'max') {
                        //* 사냥터별 최대 레벨 평타 적중률
                        mobAreaMaxLevelHitRate.value = sameRateArray[0]['same_rage']
                        mobAreaMaxLevelSkillHitRate.value = skillHitRatePercent(mobAreaMaxLevelHitRate.value)
                    }
                }
            })
    }
